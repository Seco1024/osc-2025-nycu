...
int syscall_exec(const char *file_name, char *const argv[]) {
    ...
    get_current()->code = user_program;
    get_current()->code_size = (int)file_size;

    map_pages((pagetable_t)get_current()->context.pgd, 0x0, virt_to_phys(get_current()->code), file_size, 0);
    map_pages((pagetable_t)get_current()->context.pgd, 0xffffffffb000, virt_to_phys(get_current()->stack), 0x4000, 0);
    map_pages((pagetable_t)get_current()->context.pgd, 0x3c000000, 0x3c000000, 0x1000000, 0);
...
int syscall_fork(trap_frame *frame) {
    ...
    map_pages((pagetable_t)child->context.pgd, 0x0, virt_to_phys(child->code), child->code_size, 0);
    map_pages((pagetable_t)child->context.pgd, 0xffffffffb000, virt_to_phys(child->stack), 0x4000, 0);
    map_pages((pagetable_t)child->context.pgd, 0x3c000000, 0x3c000000, 0x1000000, 0); // map mailbox
