...
task_struct *thread_create(void (*function)()) {
    ...
    thread->context.pgd = (unsigned long)allocate(4096);      // allocate a page table for the thread
...
void thread_init() {
    task_struct *idle_thread = thread_create(idle);
    idle_thread->stack = allocate(0x1000);
    idle_thread->context.sp = (unsigned long)((char*)idle_thread->stack + 0x1000);
    idle_thread->context.fp = idle_thread->context.sp;
